                ...
                initAction = delegate
                {
                    ...
                    try
                    {
                        ...
                        if (CommonUtil.GetSettings().animationExtensionEnabled)
                        {
                            ...
                            List<AnimGroupDef> playableAnimations = ContextUtil.GetPlayableAnimationsFor(participants, 'Solo LovinType goes here!');
        ...
        private IntVec3 FindPrivateSpot(Pawn pawn)
        {
            ...
            var rooms = pawn.Map.regionGrid.allRooms
                .Where(r => !r.PsychologicallyOutdoors && 
                           !r.IsHuge && 
                           r.Role != RoomRoleDefOf.DiningRoom &&
                           r.Role != RoomRoleDefOf.RecRoom);
